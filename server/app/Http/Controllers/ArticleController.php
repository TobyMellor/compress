...
class ArticleController extends Controller {
    ...
    public function index(Request $request) {
        ...
        $this->shouldCrawl($newsOutletGenreIds, $from, $to);

        $articles = Article::whereNotNull('short_sentence_summary')
                           ->whereNotNull('news_outlet_genre_id');

        if ($from) {
            $articles = Article::where('date', '>=', $from);
        } else if ($to) {
            $articles = Article::where('date', '<=', $to);
        }
     ...
     * @return void
     */
    private function shouldCrawl($newsOutletGenreIds, Carbon $from, Carbon $to) {

        // Get the Genres from $newsOutletGenreIds
        $newsOutletGenres = NewsOutletGenre::whereIn('news_outlet_genre.id', $newsOutletGenreIds)
